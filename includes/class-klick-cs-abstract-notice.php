...
if (class_exists('Klick_Cs_Abstract_Notice')) return;

/**
 * Class Klick_Cs_Abstract_Notice
 */
abstract class Klick_Cs_Abstract_Notice {
	...
	/**
	 * Klick_Cs_Abstract_Notice constructor
	 */
	...
	public function do_notice() {
		...
		return Klick_Cs()->get_dashboard()->include_template('notices-templates/' . $this->notice_template_file, false, $notice_data);
	}
	...
	public function available_at_this_time() {
		...
		$display_notice_time = Klick_Cs()->get_options()->get_option('notice-display-time');
		...
		if (!isset($display_notice_time[$this->notice_id]) || time() > $display_notice_time[$this->notice_id]) {
				...
				Klick_Cs()->get_options()->update_option('notice-display-time', $display_notice_time);
				return true;
		} else {
				return false;
		}
	...
	public function is_plugin_configured($plugin) {
		...
		return ($plugin === "wp-statistics-and-analytics/wp-statistics-and-analytics.php" && Klick_Cs()->get_options()->get_option('send-url'));
	}
}
